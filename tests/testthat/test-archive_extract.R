...
describe("archive_extract", {
  ...
  it("can handle password", {
    ...
    expect_equal(as.vector(tools::md5sum(file.path(out_dir, "iris.csv"))), as.vector(tools::md5sum(file.path(in_dir, "iris.csv"))))
    expect_equal(as.vector(tools::md5sum(file.path(out_dir, "mtcars.csv"))), as.vector(tools::md5sum(file.path(in_dir, "mtcars.csv"))))
  })
})
